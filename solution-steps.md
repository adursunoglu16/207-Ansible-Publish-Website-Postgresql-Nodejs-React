...
```
```bash
$ ansible development -m ping --key-file "~/mykey.pem"          # dynamic inventory sayesinde tag e gore istedigimize ping attik
```
```bash
$ ansible ansible_react -m ping --key-file "~/mykey.pem"
```
...
- name: Install docker
  gather_facts: No
  any_errors_fatal: true       # instalation herhangi bir error olursa durdur
  hosts: _ansible_postgresql   # ansible-inventory -i inventory_aws_ec2.yml --graph bu komutu yazinca hangi instance a install etceksek onu al
  become: true
  tasks:
    - name: upgrade all packages
      yum:                # package modulu de kullanabilirdik
        name: '*'
        state: latest
    ...
    - name: Remove docker if installed from CentOS repo
      yum:
        name: "{{ item }}"
        state: removed
      with_items:                          # bunlari hep dockerhubdan aldik
        - docker
        - docker-client
  ...
  # yum-utils is a collection of tools and programs for managing yum repositories, installing debug packages, source packages, extended information from repositories and administration.
    - name: Add Docker repo
      get_url:                                     # ansibledan baktik
        url: https://download.docker.com/linux/centos/docker-ce.repo   # bu linki dockerhubdan
